...
| 包                     | 版本   | 职责                                              | 外部依赖            |
| ---------------------- | ------ | ------------------------------------------------- | ------------------- |
| `@lessjs/core`         | 0.14.9 | DSD 渲染、Island、Navigation、日志                | parse5（唯一）      |
| `@lessjs/adapter-vite` | 0.14.9 | Vite 构建：路由扫描、Island Transform、SSG 三阶段 | vite, hono, esbuild |
| `@lessjs/adapter-lit`  | 0.14.9 | Lit TemplateResult → DSD HTML                     | lit                 |
| `@lessjs/content`      | 0.14.9 | Blog + Nav + Sitemap 构建时插件                   | marked, gray-matter |
| `@lessjs/i18n`         | 0.14.9 | 国际化 locale 展开                                | 无                  |
| `@lessjs/app`          | 0.14.9 | 伞包：lessjs() = less() + content + i18n          | —                   |
| `@lessjs/ui`           | 0.14.9 | 8 个 Web Component（layout, button, input…）      | lit                 |
| `@lessjs/signals`      | 0.14.9 | TC39 Signals polyfill + framework layer           | 无                  |
| `@lessjs/rpc`          | 0.14.9 | 零依赖 fetch RPC 控制器                           | 无                  |
| `@lessjs/create`       | 0.14.9 | 脚手架 CLI                                        | 无                  |
