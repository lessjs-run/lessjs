 ...
 * - DSD contains the full button HTML with onclick handler
 * - Lit render() always returns nothing to avoid shadow DOM conflicts
 * - State management via standalone div in light DOM
 * - Event binding via native onclick + global keyboard handler
 */
...
/**
 * Search island — DSD-first architecture.
 *
 * DSD provides the button with native onclick handler.
 * Lit provides the overlay (as a sibling in light DOM).
 * This avoids all shadow DOM hydration timing issues.
 */
export default class LessSearch extends LitElement {
  ...
  /** Open the search overlay — called from DSD onclick handler */
  open() {
    this._open = true;
    this._loadIndex();
    this.requestUpdate();
  ...
  /** Lit render: always returns nothing. DSD provides the button. */
  override render() {
