...
export class UIShowcase extends LitElement {
  static override styles = [pageStyles, css`
    ...
    .section-title { font-size: 0.625rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.16em; color: var(--less-text-muted); margin-bottom: 1.5rem; padding-bottom: 0.75rem; border-bottom: 0.5px solid var(--less-border); }
    .palette-row { display: grid; grid-template-columns: 1fr 1fr; gap: 1px; background: var(--less-border); border: 0.5px solid var(--less-border); border-radius: 6px; overflow: hidden; }
    .palette-card { padding: 1.5rem; }
    .palette-dark { background: var(--less-bg-base); }
    .palette-light { background: #fff; }
    .palette-name { font-size: 0.625rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.12em; margin-bottom: 1rem; }
    .palette-dark .palette-name { color: var(--less-text-muted); }
    .palette-light .palette-name { color: var(--less-text-secondary); }
    .swatch-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 0.75rem; margin-bottom: 1.25rem; }
    .swatch-item { text-align: center; }
    .swatch { width: 100%; aspect-ratio: 1; border-radius: 4px; margin-bottom: 0.375rem; }
    .palette-dark .swatch { border: 1px solid rgba(255, 255, 255, 0.08); }
    .palette-light .swatch { border: 1px solid rgba(0, 0, 0, 0.08); }
    .swatch-label { font-size: 0.5625rem; font-weight: 600; letter-spacing: 0.04em; }
    .palette-dark .swatch-label { color: var(--less-text-muted); }
    .palette-light .swatch-label { color: var(--less-text-secondary); }
    .palette-desc { font-size: 0.75rem; line-height: 1.6; }
    .palette-dark .palette-desc { color: var(--less-text-tertiary); }
    .palette-dark .palette-desc strong { color: var(--less-text-primary); }
    .palette-light .palette-desc { color: var(--less-text-secondary); }
    .palette-light .palette-desc strong { color: var(--less-text-primary); }
    .type-scale { display: flex; flex-direction: column; }
    .type-row { display: flex; align-items: baseline; gap: 1.5rem; padding: 0.75rem 0; border-bottom: 0.5px solid var(--less-border); }
    .type-row:last-child { border-bottom: none; }
    .type-label { min-width: 72px; font-size: 0.5625rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.1em; color: var(--less-text-muted); }
    .type-sample { color: var(--less-text-primary); }
    .preview-card { background: var(--less-bg-surface); border: 0.5px solid var(--less-border); border-radius: 6px; overflow: hidden; }
    .preview-header { padding: 0.875rem 1.25rem; border-bottom: 0.5px solid var(--less-border); display: flex; justify-content: space-between; align-items: center; }
    .preview-title { font-size: 0.8125rem; font-weight: 600; color: var(--less-text-primary); }
    .preview-badge { font-size: 0.5625rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.08em; padding: 0.25rem 0.5rem; border-radius: 3px; background: var(--less-accent-subtle); color: var(--less-text-secondary); border: 0.5px solid var(--less-border); }
    .preview-body { padding: 1.25rem; display: flex; gap: 0.625rem; flex-wrap: wrap; align-items: flex-start; }
    .cards-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem; }
    .install-section { margin-top: 3.5rem; padding: 2rem; background: var(--less-bg-surface); border: 0.5px solid var(--less-border); border-radius: 6px; text-align: center; }
    .install-section h3 { font-size: 0.9375rem; font-weight: 600; color: var(--less-text-primary); margin: 0 0 1rem; }
    .install-cmd { display: inline-flex; align-items: center; gap: 0.625rem; padding: 0.625rem 1.25rem; background: var(--less-bg-elevated); border: 0.5px solid var(--less-border); border-radius: 4px; font-family: "SF Mono", monospace; font-size: 0.8125rem; color: var(--less-text-primary); }
    .install-cmd .prompt { color: var(--less-text-muted); }
    .install-section p { font-size: 0.8125rem; color: var(--less-text-tertiary); margin: 0.75rem 0 0; }
    @media (max-width: 900px) { .section { margin-bottom: 2.5rem; } .type-row { gap: 1rem; } .preview-body { padding: 1rem; } .install-section { padding: 1.5rem 1rem; } }
    @media (max-width: 640px) { .palette-row { grid-template-columns: 1fr; } .swatch-grid { grid-template-columns: repeat(3, 1fr); gap: 0.5rem; } .install-cmd { font-size: 0.75rem; padding: 0.5rem 1rem; } }
  `];
