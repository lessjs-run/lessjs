...
app.post('/', async (c) => {
  ...
  switch (cmd) {
    ...
    case 'version':
      output = ['<span style="color:#86efac;">v0.13.0</span> — api convergence + phase checks'];
      break;
    ...
    case 'uname':
      output = ['<span style="color:#86efac;">lessjs</span> <span style="color:#52525b;">deno 2.7+ node 18+ edge</span>'];
      break;
    ...
    default:
      output = [`<span style="color:#ef4444;">command not found:</span> ${cmd}. type <span style="color:#fbbf24;">help</span> for available commands.`];
  }
