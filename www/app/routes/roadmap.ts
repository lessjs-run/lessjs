  ...
  private _renderZh() {
    return html`
      ...
      <p class="subtitle">
        ...
        <code>0.14.6</code>，但下一阶段必须先修复发布一致性、package island SSR、per-page
        island manifest 和 create scaffold 入口，再扩大 Web Components registry hub 叙事。
      </p>

      <div class="callout">
        <p>
          当前原则是事实优先、标准优先、协议优先：已经被测试证明的能力进入文档；尚未冻结的能力留在 roadmap 和 ADR；
          registry hub 只能建立在 renderer kernel、package manifest 和 release parity 稳定之后。
          版本纪律继续遵循 ADR 0006，npm 发布策略继续遵循 ADR 0007，2026-05-15 的执行 SOP 见 ADR 20260515-1。
        </p>
      </div>
      ...
      <p>
        ...
        OpenWC，也不是先做中心化 marketplace，而是把 LessJS 自己已经具备的 DSD 渲染、SSR bundle、SSG pipeline、
        package island model 和发布纪律产品化。
      </p>
      ...
      <table class="version-table">
        ...
        <tbody>
          ...
          <tr>
            <td>v0.14-v0.14.6</td>
            <td>确立 DSD/WC 为主叙事，完成 docs 自举、发布纪律、标准审计入口、安全补丁和 main 线鲁棒性修补。</td>
            <td>Done</td>
          </tr>
        </tbody>
      </table>
      ...
      <div class="phase">
        ...
        <p>目标：先让 install、scaffold、package SSR 和 SSG manifest 真实可靠，再冻结 Web Component render protocol。</p>
        <ul class="criteria-list">
          <li>修复 <code>@lessjs/create</code> 的 JSR metadata endpoint、<code>adapter-vite</code> 版本解析、<code>--help</code> 和项目名校验。</li>
          <li>对齐 <code>deno.lock</code>、package versions、changelog 和 registry latest，发布前后都有自动检查。</li>
          <li>让 package islands 在 SSR bundle 中被显式导入和注册，不能只参与 client upgrade。</li>
          <li>接通 <code>buildIslandChunkMap()</code>、<code>generateIslandManifests()</code> 和 <code>writeIslandManifests()</code>，或明确推迟页面级 manifest 文档。</li>
      ...
      <div class="phase">
        ...
        <ul class="criteria-list">
          ...
          <li>在 Custom Elements Manifest 兼容字段之上增加 LessJS 的 <code>ssr</code>、<code>dsd</code>、<code>hydrate</code> 和 diagnostics 字段。</li>
      ...
      <div class="phase">
        ...
        <ul class="criteria-list">
          ...
          <li>建立本地 registry 页面原型，展示组件 manifest、SSR/SSG 结果、bundle cost、测试状态和文档索引能力。</li>
  ...
  private _renderEn() {
    return html`
      ...
      <p class="subtitle">
        ...
        <code>origin/main</code>: source packages are at <code>0.14.6</code>, but the next
        phase must fix release parity, package island SSR, per-page island manifests,
        and the create scaffold before expanding the Web Components registry hub story.
      </p>

      <div class="callout">
        <p>
          The roadmap is fact-first, standards-first, and protocol-first. Tested capabilities
          move into docs; unfinished boundaries stay in the roadmap and ADRs; the registry hub
          only follows renderer kernel, package manifest, and release parity stability. Version
          discipline follows ADR 0006, npm publishing follows ADR 0007, and the 2026-05-15
          execution SOP is ADR 20260515-1.
        </p>
      </div>
      ...
      <p>
        ...
        and rendering infrastructure. The near-term goal is not to follow OpenWC or lead
        with a centralized marketplace, but to productize LessJS's own DSD rendering,
        SSR bundle, SSG pipeline, package island model, and release discipline.
      </p>
      ...
      <table class="version-table">
        ...
        <tbody>
          ...
          <tr>
            <td>v0.14-v0.14.6</td>
            <td>Made DSD/WC the primary narrative and added docs self-hosting, release discipline, standards audit entry points, safety patches, and mainline robustness fixes.</td>
            <td>Done</td>
          </tr>
        </tbody>
      </table>
      ...
      <div class="phase">
        <span class="status next">Next</span>
        <h3>v0.15 Renderer Kernel &amp; Release Parity</h3>
        <p>Goal: make install, scaffold, package SSR, and SSG manifests reliable before freezing the Web Component render protocol.</p>
        <ul class="criteria-list">
          <li>Fix <code>@lessjs/create</code> JSR metadata endpoint, <code>adapter-vite</code> version resolution, <code>--help</code>, and project-name validation.</li>
          <li>Align <code>deno.lock</code>, package versions, changelog, and registry latest with automated checks before and after publish.</li>
          <li>Explicitly import and register package islands in the SSR bundle instead of using them only for client upgrade.</li>
          <li>Wire <code>buildIslandChunkMap()</code>, <code>generateIslandManifests()</code>, and <code>writeIslandManifests()</code>, or clearly defer page-level manifest docs.</li>
          ...
          <li>Leave room for non-Lit adapters without promising a full adapter matrix or centralized hub before the protocol stabilizes.</li>
        </ul>
      </div>
      ...
      <div class="phase">
        <span class="status planned">Planned</span>
        <h3>v0.16 WC Package Protocol</h3>
        <p>Goal: extend <code>PackageIslandMeta</code> into a CEM-compatible component package manifest.</p>
        <ul class="criteria-list">
          <li>Declare tag, module, SSR renderability, upgrade strategy, parts, states, events, and tokens.</li>
          <li>Add LessJS <code>ssr</code>, <code>dsd</code>, <code>hydrate</code>, and diagnostics fields on top of Custom Elements Manifest-compatible metadata.</li>
      ...
      <div class="phase">
        ...
        <ul class="criteria-list">
          ...
          <li>Create a local registry page prototype that demonstrates component manifests, SSR/SSG results, bundle cost, test status, and docs indexing.</li>
      ...
      <div class="phase">
        <span class="status planned">Planned</span>
        <h3>v0.18 Hub Prototype &amp; v1.0 API Freeze</h3>
        <p>Goal: publish the static hub prototype first, then freeze the public boundaries that actually support 1.0.</p>
        <ul class="criteria-list">
          <li>The public hub consumes manifests and validation artifacts; it does not trust README or marketing fields directly.</li>
          <li>Define package provenance, review, reporting, security response, and maintenance boundaries before marketplace behavior.</li>
