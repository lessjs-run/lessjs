...
export class IslandsDeepGuidePage extends LitElement {
  ...
  private _renderZh() { return html`<less-layout locale="${this.locale||'zh'}" .locales="${['en','zh']}" .navItems="${navSections}" .headerNav="${headerNav}" current-path="/guide/islands-deep"><div class="container">
    <h1>Island 深度指南</h1>
    <p class="subtitle">Island 是 LessJS 中唯一允许的客户端 JavaScript 单元。本页深入讲解 Island 的三层架构、升级策略、声明式事件绑定和数据传递机制。</p>
    <h2>Island 架构原理</h2>
    <p>LessJS 的 Island 是对 Custom Element Upgrade 机制的诚实利用。浏览器解析 HTML 时看到 <span class="inline-code">&lt;my-counter&gt;</span>，稍后加载模块调用 <span class="inline-code">customElements.define()</span>，已有元素被升级——这就是 Island 的全部原理。关于 DSD 渲染模型和为什么选择 DSD-first，参见 <a href="/guide/dsd">DSD 渲染架构</a>。</p>
    <h2>三层 Island 架构</h2>
    ...
    <div class="nav-row"><a href="/guide/dsd" class="nav-link">&larr; DSD 渲染架构</a><a href="/guide/rpc" class="nav-link">RPC 远程调用 &rarr;</a></div>
  </div></less-layout>`; }

  private _renderEn() { return html`<less-layout locale="${this.locale||'en'}" .locales="${['en','zh']}" .navItems="${navSections}" .headerNav="${headerNav}" current-path="/en/guide/islands-deep"><div class="container">
    ...
    <p>LessJS islands are a straightforward exploitation of the Custom Element Upgrade mechanism. The browser sees <span class="inline-code">&lt;my-counter&gt;</span> during HTML parsing; later the module loads and calls <span class="inline-code">customElements.define()</span>, upgrading the existing element. See <a href="/guide/dsd">DSD Architecture</a> for the rendering model.</p>
    ...
    <div class="nav-row"><a href="/guide/dsd" class="nav-link">&larr; DSD Architecture</a><a href="/guide/rpc" class="nav-link">RPC &rarr;</a></div>
  </div></less-layout>`; }
}
