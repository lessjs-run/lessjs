...
export class DeploymentPage extends LitElement {
  ...
  private _renderZh() { return html`<less-layout locale="${this.locale||'zh'}" .locales="${['en','zh']}" .navItems="${filterFrameworkNav(navSections)}" .headerNav="${headerNav}" current-path="/guide/deployment"><div class="container">
    ...
    <p>构建输出 <span class="inline-code">dist/</span>：带 Declarative Shadow DOM 的静态 HTML、client island chunks 和复制的公开资源。</p>
    <h2>Static Hosting</h2>
    <div class="platform-grid">
      <div class="platform-card"><h3>GitHub Pages</h3><p>部署在仓库子路径下时，设置 Vite base。</p></div>
      <div class="platform-card"><h3>Cloudflare Pages</h3><p>构建命令：deno task build；输出目录：dist。</p></div>
      <div class="platform-card"><h3>Netlify</h3><p>发布目录：dist。</p></div>
      <div class="platform-card"><h3>Vercel</h3><p>使用静态输出，Framework 预设选 "Other"。</p></div>
      <div class="platform-card"><h3>S3 / CloudFront</h3><p>上传 dist 并配置合适的缓存头。</p></div>
    </div>
    <h2>API Deployment</h2>
    <p>API 路由属于生成的 Hono app。静态托管不会自动执行它们。当应用需要运行时行为时，通过平台 adapter 部署 API 路由。</p>
    <table><thead><tr><th>Target</th><th>Status</th><th>Notes</th></tr></thead><tbody>
      <tr><td>Deno Deploy</td><td>Natural fit</td><td>Closest to the Deno-first development model.</td></tr>
      <tr><td>Cloudflare Workers</td><td>Good fit</td><td>Hono maps well to Workers.</td></tr>
      <tr><td>Vercel / Netlify Functions</td><td>Needs adapter</td><td>Requires documented build output and runtime entry contract.</td></tr>
    </tbody></table>
    <h2>No Production SSR Server by Default</h2>
    <p>LessJS 主路径不需要长期运行的生产 SSR 服务器。静态页面应保持静态；动态行为应为显式 API 或未来的 ISR。这使托管便宜、可缓存、运维轻量。</p>
  ...
  private _renderEn() { return html`<less-layout locale="${this.locale||'en'}" .locales="${['en','zh']}" .navItems="${filterFrameworkNav(navSections)}" .headerNav="${headerNav}" current-path="/en/guide/deployment"><div class="container">
    ...
    <h2>Build Once</h2>
    <less-code-block><pre><code>deno task build</code></pre></less-code-block>
    <p>The build outputs <span class="inline-code">dist/</span>: static HTML with Declarative Shadow DOM, client island chunks, and copied public assets.</p>
    <h2>Static Hosting</h2>
    <div class="platform-grid">
      <div class="platform-card"><h3>GitHub Pages</h3><p>Set Vite base when deploying under a repo sub-path.</p></div>
      <div class="platform-card"><h3>Cloudflare Pages</h3><p>Build command: deno task build; Output dir: dist.</p></div>
      <div class="platform-card"><h3>Netlify</h3><p>Publish directory: dist.</p></div>
      <div class="platform-card"><h3>Vercel</h3><p>Use static output, Framework preset: "Other".</p></div>
      <div class="platform-card"><h3>S3 / CloudFront</h3><p>Upload dist/ with appropriate cache headers.</p></div>
    </div>
    <h2>API Deployment</h2>
    <p>API routes belong to the generated Hono app. Static hosting doesn't execute them. Deploy API routes via platform adapters when runtime behavior is needed.</p>
    <h2>No Production SSR Server by Default</h2>
    <p>LessJS's main path doesn't need a long-running production SSR server. Static pages stay static; dynamic behavior should be explicit API or future ISR. This keeps hosting cheap, cacheable, and operationally lightweight.</p>
    <h2>Deployment Checklist</h2>
