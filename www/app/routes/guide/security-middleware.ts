...
export class SecurityMiddlewarePage extends LitElement {
  static override styles = [pageStyles, css`.chain { padding: 1rem; background: var(--less-bg-surface); border-left: 2px solid var(--less-border-hover); border-radius: 0 4px 4px 0; margin: 1rem 0; font-family: "SF Mono", monospace; font-size: 0.8125rem; line-height: 1.8; color: var(--less-text-secondary); white-space: pre-wrap; }`];
  override render() { return (this.locale||'zh')==='en'?this._renderEn():this._renderZh(); }

  private _renderZh() { return html`<less-layout locale="${this.locale||'zh'}" .locales="${['en','zh']}" .navItems="${filterFrameworkNav(navSections)}" .headerNav="${headerNav}" current-path="/guide/security-middleware"><div class="container">
  ...
  private _renderEn() { return html`<less-layout locale="${this.locale||'en'}" .locales="${['en','zh']}" .navItems="${filterFrameworkNav(navSections)}" .headerNav="${headerNav}" current-path="/en/guide/security-middleware"><div class="container">
