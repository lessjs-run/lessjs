...
`@lessjs/core` now exports only 6 public subpaths:

```ts
// Remaining (public)
import { ... } from '@lessjs/core';              // runtime utilities
import { LessError } from '@lessjs/core/errors';
import { createSsrContext } from '@lessjs/core/context';
import { createLogger } from '@lessjs/core/logger';
import { navigate } from '@lessjs/core/navigation';
...
Generated code resolves through `CORE_SUBPATHS` mapping in `@lessjs/adapter-vite`.

### ssr-handler.ts deleted

The pure re-export facade `ssr-handler.ts` is finally gone. Consumers import directly from `@lessjs/core`.

### Virtual module IDs moved

All Vite virtual module constants (`VIRTUAL_NAV_ID`, `RESOLVED_BLOG_DATA_ID`, etc.) moved from `@lessjs/core/constants` to `@lessjs/adapter-vite/virtual-ids`. Core no longer knows about Vite internals.
...
| `@lessjs/signals`       | 0.6.2   | 0.6.3      |
| `@lessjs/app`          | 0.3.0   | 0.3.1      |
| `@lessjs/i18n`         | 0.1.0   | 0.1.1      |
| `@lessjs/create`       | 0.8.0   | 0.8.1      |
...
### If you use `@lessjs/app` (recommended)

**No changes needed.** `lessjs()` abstracts all internal restructuring.

### If you import from `@lessjs/core` subpaths

Remove imports from removed subpaths:

```ts
// Delete these:
import { ... } from '@lessjs/core/render-dsd';    // ❌ removed
import { ... } from '@lessjs/core/html-escape';    // ❌ removed
import { ... } from '@lessjs/core/adapter-registry'; // ❌ removed
import { ... } from '@lessjs/core/ssr-handler';    // ❌ removed in v0.12
```

All runtime exports are available from the main entry:

```ts
import { escapeHtml, registerAdapter, renderDSD } from '@lessjs/core'; // ✅
```
