...
draft: false
excerpt: 'Eliminated all module-level state from @lessjs/content and @lessjs/i18n. Pure functions replace stateful init/getter patterns. Virtual modules become the only data bridge for SSR routes.'
---
...
The core idea: `@lessjs/content` and `@lessjs/i18n` must not hold module-level state (`_posts`, `_options`). Instead, data flows through Vite virtual modules that read from `LessBuildContext` at `load()` time.
...
These stateful APIs on `@lessjs/content` are gone. No backward compatibility — v0.12.0 is a structured breaking change.

```ts
// Before (v0.11.x) — module state
import { initBlogData, getPosts } from '@lessjs/content';
...
- `import.meta.resolve('@lessjs/core')` → `file:///path/to/packages/core/index.ts`
...
```ts
// Route component
import { locales, getDefaultLocale } from 'virtual:less-i18n-data';
```
