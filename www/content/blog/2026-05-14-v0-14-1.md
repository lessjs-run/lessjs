...
- The signals package name is unified as `@lessjs/signals`.
- `@lessjs/adapter-vite` exposes CLI subpaths for `build`, `build-client`, and `build-ssg`.
- `@lessjs/create` generates versioned build tasks that target the public adapter-vite CLI exports.
...
The current stable API candidates are `lessjs()` options, public `@lessjs/core` exports, `@lessjs/adapter-vite` CLI subpaths, `@lessjs/rpc`, `@lessjs/signals`, and `@lessjs/ui` component subpaths.
