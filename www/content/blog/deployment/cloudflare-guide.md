...
`functions/api/term.ts` 放在仓库根目录（与 `src-tmp/`、`my-tech-blog/` 同级）。Cloudflare Pages 在部署时会自动扫描根目录下的 `functions/` 目录，将其中的文件编译为 serverless functions，无需额外配置。

> **重要：** `functions/` 必须在根目录（与 `src-tmp/` 同级），不能在 `src-tmp/` 内部。Cloudflare 的根目录为空时，只检测 repo 根级别的 `functions/`。
