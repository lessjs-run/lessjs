...
const checks: Check[] = [
  ...
  {
    name: 'v0.20.0 is the current public line',
    files: currentDocs,
    required: ['v0.20.0'],
  },
  {
    name: 'v0.21 is the next framework credibility line',
    files: [
      'README.md',
      'README.en.md',
      'docs/roadmap/ROADMAP.md',
  ...
  {
    name: 'planned framework work is not described as shipped',
    files: publicDocs,
    forbidden: [
      /client:load[,/ ].*client:idle[,/ ].*client:visible[,/ ].*client:only.*(shipped|available|stable|implemented)/i,
      /(ISR|request-time SSR)\s+(is|are)\s+(shipped|stable|implemented)/i,
