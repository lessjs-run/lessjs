...
Deno.test('lessPlugin: virtual-entry resolves virtual:less-hono-entry', () => {
  ...
  const resolved = (virtualPlugin.resolveId as (id: string) => unknown)('virtual:less-hono-entry');
...
Deno.test('lessPlugin: virtual-entry resolveId returns undefined for unknown IDs', () => {
  ...
  const result = (virtualPlugin.resolveId as (id: string) => unknown)('some-random-module');
...
Deno.test('lessPlugin: virtual-entry load returns code for resolved entry ID', () => {
  ...
  const code = (virtualPlugin.load as (id: string) => unknown)('\0virtual:less-hono-entry');
  assertExists(code);
...
Deno.test('lessPlugin: virtual-entry load returns null export for build trigger', () => {
  ...
  const code = (virtualPlugin.load as (id: string) => unknown)('\0virtual:less-build-trigger');
  assertExists(code);
...
Deno.test('lessPlugin: virtual-entry load returns undefined for unknown IDs', () => {
  ...
  const result = (virtualPlugin.load as (id: string) => unknown)('unknown-virtual-id');
  assertEquals(result, undefined);
});
...
Deno.test('lessPlugin: core config sets chunkSizeWarningLimit', () => {
  ...
  const result = (corePlugin.config as (id: string) => unknown)({}) as Record<string, unknown>;
