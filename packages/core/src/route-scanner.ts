...
export async function scanRoutes(
  ...
  try {
    ...
    console.debug(`[LessJS] Routes directory "${routesDir}" not found: ${e instanceof Error ? e.message : String(e)}`);
    return entries;
  }
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.debug(`[LessJS] File vanished before stat: ${fullPath}`, e instanceof Error ? e.message : '');
      continue;
    }
...
export async function scanIslands(
  ...
  try {
    ...
    // Directory doesn't exist yet — return empty
    console.debug(`[LessJS] Islands directory "${islandsDir}" not found: ${e instanceof Error ? e.message : String(e)}`);
    return files;
  }
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      console.debug(`[LessJS] Island file vanished before stat: ${fullPath}`, e instanceof Error ? e.message : '');
      continue;
    }
