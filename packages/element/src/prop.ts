...
const _propSignals = new WeakMap<OpenElement, PropSignalMap>();
const _propUnsubscribers = new WeakMap<OpenElement, Array<() => void>>();
const _ctorMetadata = new WeakMap<object, PropMetadataStore>();
const _staticPropSignals = new WeakMap<OpenElement, Map<string, PropSignal>>();
const _staticPropUnsubs = new WeakMap<OpenElement, Array<() => void>>();

// ─── @prop() runtime (legacy compat) ────────────────────────────

export function initializeProps(instance: OpenElement): void {
  ...
  registerObservedAttributes(
    instance.constructor as typeof OpenElement & { observedAttributes?: string[] },
    store,
  );
}

export function disposeProps(instance: OpenElement): void {
...
function registerObservedAttributes(
  ctor: typeof OpenElement & { observedAttributes?: string[] },
...
export function initializeStaticProps(instance: OpenElement): void {
...
export function disposeStaticProps(instance: OpenElement): void {
...
export function syncStaticPropsFromAttributes(instance: OpenElement): void {
  const ctor = instance.constructor as { props?: Record<string, unknown> };
  const propsDef = ctor.props as Record<string, unknown> | undefined;
  if (!propsDef) return;
