/**
 * @openelement/adapter-react - SSR Adapter
 *
 ...
 *
 * @module @openelement/adapter-react/ssr
 */
...
export async function renderReactToString(
  ...
  try {
    ...
    const tag = tagName || 'unknown';
    throw new Error(
      `[openElement] <${tag}> React SSR rendering failed. ` +
...
/**
 * Install the React SSR adapter into @openelement/core's renderDsd.
 *
