...
| **Signals**                | `@openelement/signals` (TC39 proposal-aligned) | `@openelement/core` (DsdElement reactive)       |
...
| Signals                | 一等公民   | 不做任何处理 | `@openelement/signals`（实现 TC39 proposal，非封装） | Signals 是 JS 库，ESM import              |
...
3. **类型安全**：如 `@openelement/signals` 提供类型化的 Signal API
...
│  @openelement/signals ← TC39-aligned implementation,              │
