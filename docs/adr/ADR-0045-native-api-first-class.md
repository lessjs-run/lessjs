...
| API | 使用方式 | 涉及模块 |
|-----|---------|---------|
| **Declarative Shadow DOM** | `shadowrootmode="open"` 属性，`DsdElement` | `@lessjs/core`, `@lessjs/adapter-lit` |
| **CSS Parts** | `::part()` 选择器 | `@lessjs/ui` (less-button, less-card 等) |
| **Custom Elements** | `customElements.define()` | 所有组件 |
| **HTML Template** | `<template>` 元素 | DSD 渲染管线 |
| **Signals** | `@lessjs/signals` (TC39 proposal-aligned) | `@lessjs/core` (DsdElement reactive) |
...
| Signals | 一等公民 | 不做任何处理 | `@lessjs/signals`（实现 TC39 proposal，非封装） | Signals 是 JS 库，ESM import |
| View Transitions | 一等公民 | 不做任何处理 | 无（直接注入 `<meta>` 标签） | Meta 标签，无 JS API 调用 |
| `CSSStyleSheet` | 有条件封装 | 不做任何处理 | `@lessjs/core` StyleSheet（SSR 兼容层） | SSR 环境需要 polyfill；客户端直接使用原生 |
...
3. **类型安全**：如 `@lessjs/signals` 提供类型化的 Signal API
...
│  @lessjs/signals ← TC39-aligned implementation,              │
...
export default class MyGreeting extends DsdElement {
  ...
  override render() {
    ...
    // 开发者只需返回模板内容
    return html`<h1 part="heading">Hello, ${this.name}!</h1>`;
  }
}
...
- **不影响已有组件**：现有 `DsdElement`、`@lessjs/ui` 组件的 API 不变
- **Lit 适配器不受影响**：`@lessjs/adapter-lit` 继续封装 Lit 的 CSS 和生命周期
