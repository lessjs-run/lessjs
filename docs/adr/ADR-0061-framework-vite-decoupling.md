...
| 虚拟模块                 | 消费方                                      | 提供方                                       |
| ------------------------ | ------------------------------------------- | -------------------------------------------- |
| `virtual:less-nav`       | 55 个路由文件                               | `@openelement/content` 的 `virtualNavPlugin` |
| `virtual:less-blog-data` | 5 个路由文件 + `entry-renderer.ts` 生成代码 | `@openelement/content` 的 `blog-data-plugin` |
| `virtual:less-i18n-data` | `entry-renderer.ts` 生成代码                | `@openelement/i18n` 的 `i18n-data-plugin`    |

### 问题

1. **框架层依赖构建工具**：路由组件必须通过 Vite 虚拟模块机制获取导航/博客/i18n 数据，离开 Vite 环境无法运行。这与 ADR-0050 确立的 Layered Package Architecture 矛盾——框架包 `@openelement/content` 和 `@openelement/i18n` 的数据应对构建工具透明。
...
框架层（纯 ESM，零构建工具依赖）
    ↓
数据层（框架承包导出，@openelement/content/nav, @openelement/i18n/data）
    ↓
...
`@openelement/content` 新增以下子路径：

```
@openelement/content/nav   → 导出 navSections, headerNav（来自 scanNavData 结果）
@openelement/content/blog  → 导出 posts, getPostBySlug, getBlogOptions（来自 loadBlogData 结果）
```

`@openelement/i18n` 新增以下子路径：

```
@openelement/i18n/data  → 导出 locales, getDefaultLocale, getI18nOptions（来自 loadI18nData 结果）
```
...
- **55 个路由文件**：`import { navSections } from 'virtual:less-nav'` → `import { navSections } from '@openelement/content/nav'`
- **5 个路由文件**：`import { posts, getPostBySlug } from 'virtual:less-blog-data'` → `import { posts, getPostBySlug } from '@openelement/content/blog'`
- **entry-renderer.ts 生成代码**：不再生成含 `virtual:less-blog-data` 和 `virtual:less-i18n-data` 的 import 语句，改用 `@openelement/content/blog` 和 `@openelement/i18n/data`

迁移通过自动化 codemod 脚本执行，保证一致性。

#### 第三步：移除虚拟模块基础设施（P1）

- 移除 `@openelement/content` 中的 `virtualNavPlugin` 和 `blog-data-plugin` 的虚拟模块注册逻辑
- 移除 `@openelement/i18n` 中的 `i18n-data-plugin` 的虚拟模块注册逻辑
- 移除 `@openelement/adapter-vite` 中对应虚拟模块的 `resolveId`/`load` 处理
- `@openelement/protocols/virtual-ids` 中移除对应的常量

#### 向后兼容策略

- v0.26.0：Vite 插件保留 `virtual:less-nav` → `@openelement/content/nav` 的 alias 映射（deprecated warning）
...
5. **包依赖图改善**：`@openelement/content` 和 `@openelement/i18n` 不再需要与 Vite 插件机制耦合
...
{
  "imports": {
    "@openelement/content/nav": "./packages/content/src/nav/index.ts"
  }
}
```
...
Vite 插件将 `virtual:less-nav` 映射到 `@openelement/content/nav` 的 re-export。
...
1. `@openelement/content` 新增 `/nav`、`/blog` 子路径导出
2. `@openelement/i18n` 新增 `/data` 子路径导出
...
2. 移除 `@openelement/content` 和 `@openelement/i18n` 中的虚拟模块插件代码
3. 移除 `@openelement/protocols/virtual-ids` 中废弃的常量

---

## Acceptance Criteria

- [ ] `@openelement/content/nav` 导出 `navSections`, `headerNav`
- [ ] `@openelement/content/blog` 导出 `posts`, `getPostBySlug`, `getBlogOptions`
- [ ] `@openelement/i18n/data` 导出 `locales`, `getDefaultLocale`, `getI18nOptions`
