...
- **Counter stuck at 42**: `data-signal` signal binding lost in `_layoutWorkaroundReRender` + `effectScope` nesting
...
| **LessJS (target)** | VNode → `data-signal` HTML                          | `querySelector('[data-signal]')` → signal binding | **Preserved in DOM attributes** |
...
LessJS's `renderToString` currently strips signal identity. Without `data-signal` attributes, hydration relies on fragile position-guessing via `_walkAndBind`.
...
SSR (VNode→HTML) and CSR (VNode→DOM) are two serialization paths over the same VNode tree. Signal identity is preserved in SSR HTML via `data-signal` attributes. Client hydration reads attributes to establish signal→DOM bindings — zero position guessing.
...
render() → VNode (ONE representation)
              ...
              │         signal props → data-signal="name" on element
...
hydration: querySelectorAll('[data-signal]')
            → read data-signal → signalRegistry.get(name) → effect(el.prop = sig.value)
```

### Key Decisions

1. **`data-signal` attribute** for signal identity in SSR HTML
2. **`data-on` attribute** for event identity in SSR HTML
3. **`signalRegistry`** on DsdElement — maps signal names to signal objects accessible by hydration code
4. **`hydrateSignals(root)`** replaces `_walkAndBind` traversal — `querySelectorAll('[data-signal]')` + effect creation
5. **`hydrateEvents(root)`** replaces `_walkAndBind` event binding — `querySelectorAll('[`data-on`]')` + addEventListener
...
3. **Counter works**: `data-signal="count"` → `effect(el.textContent = signal.value)` — one line per signal, reliably triggered.
...
6. **Future-proof**: `data-signal` attributes pave the way for signal-driven conditional rendering (`{signal && <div>}`) — metadata is already in the DOM.

### Negative

1. **HTML payload increases** by `data-signal="name"` per signal binding (negligible — ~20 bytes each, GZIP'd to ~5 bytes)
...
| `jsx-render-string.ts` | `textContent` child content logic (replaced by `data-signal` attribute generation) |
...
| `dsd-element.ts`       | `hydrateSignals(root)` — `querySelectorAll('[data-signal]')` iterate + effect create |
| `dsd-element.ts`       | `hydrateEvents(root)` — `querySelectorAll('[`data-on`]')` iterate + addEventListener |
| `jsx-render-string.ts` | `serializeAttrs` — `textContent` prop → `data-signal="signalName"` attribute         |
