...
Application (www/)           消费: UI + Content + Framework
    ↓
UI (@openelement/ui)              消费: Content + Framework + Style
    ↓
Content (@openelement/content)    消费: Framework
    ↓
Framework (@openelement/core + adapter-vite)  消费: 无（底层）
```
...
@openelement/content
  ...
  _generated-nav.ts         ← 导出 navSections / headerNav
        ↓
  www/deno.json import map  ← "@openelement/content/nav": "./app/data/_generated-nav.ts"
        ↓
@openelement/ui\/open-layout.tsx
  └─ import { navSections, headerNav } from '@openelement/content/nav'
        ↓
...
- `@openelement/content/nav` 是一个内容包的子导出路径
- `less-layout` 从它 import 数据，这是正常的包依赖关系
- 如果某个项目不用 `less-layout`，nav 数据仍然可以被任何其他组件 import
- 如果 `@openelement/content/nav` 不可用，`less-layout` fallback 到空数组
...
| 文件:行号                                 | 删除内容                     | 原因                            |
| ----------------------------------------- | ---------------------------- | ------------------------------- |
| `ui\/open-layout.tsx:553-568`             | `_manifestData<T>(key)` 方法 | UI 组件不应读 globalThis 拿数据 |
...
| `ui\/open-layout.tsx:532`                | `this._manifestData<NavSection[]>('navSections')` 调用  | 改为 fallback 到 `[]` 或直接 import |
| `ui\/open-layout.tsx:546`                | `this._manifestData<HeaderNavLink[]>('headerNav')` 调用 | 同上                                |

### 导入删除

| 文件:行号                            | 删除内容                                                                         |
| ------------------------------------ | -------------------------------------------------------------------------------- |
| `ui\/open-layout.tsx:34`             | 如果有 `import { ... } from '@openelement/content/nav'` 但不应有 `createContext` |
...
import { navSections as _navSections, headerNav as _headerNav } from '@openelement/content/nav';
...
private _rawNavItems(): NavSection[] {
  try {
    ...
    // Default: directly from @openelement/content/nav
...
1. `<open-layout>` 上的 `locales` 属性（SSR 输出中保留）
...
**文件**: `packages/ui/src\/open-layout.tsx`
...
- 在文件顶部新增 `import { navSections as _navSections, headerNav as _headerNav } from '@openelement/content/nav';`
...
- locale 列表从 `<open-layout locales='["en","zh"]'>` 属性获取（SSR 时注入）
...
- 确认 `"@openelement/content/nav": "./app/data/_generated-nav.ts"` 仍然存在且正确
- 如有 `@openelement/content/nav` 的其他 import map 引用，确认路径
...
- [ ] `packages/ui/src\/open-layout.tsx` 中 `_manifestData()` 方法已删除
...
- [ ] `less-layout` 的 import 来自 `@openelement/content/nav`（而非 globalThis）
...
| 包                  | 影响                       | 风险                    |
| ------------------- | -------------------------- | ----------------------- |
| @openelement/ui     | less-layout 的数据来源改变 | 低 — import 是标准 JS   |
| @openelement/router | 移除 manifest fallback     | 低 — URL 推断是标准做法 |
