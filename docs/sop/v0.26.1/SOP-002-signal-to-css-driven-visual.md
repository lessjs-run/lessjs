...
| 文件                                       | 改动                              | 目的                       |
| ------------------------------------------ | --------------------------------- | -------------------------- |
| `packages/ui/src/open-props-tokens.ts`     | `:host { display: block }` 加入   | 所有组件默认 block 布局    |
| `www/app/routes/index/index.tsx`           | `heroSheet` 加 `:host` 规则       | docs-home 在 DSD 模板中有 block |
| `packages/adapter-vite/src/ssg-postprocess.ts` | CSS 注入 `docs-home{display:block}` | 全局兜底                   |
| `packages/core/src/dsd-element.ts`         | `this.style.display = 'block'`    | 升级时 inline 强制 block   |
| `packages/core/src/dsd-element.ts`         | `effect(() => render())` hotfix   | 临时止血（Phase 3 删除）   |

### 验证

- `getBoundingClientRect().height > 0` 对 `docs-home` 和 `less-layout`
- 全页截图 + viewport 截图均包含可见内容
- 0 JS errors（`Unexpected identifier 'as'` 已单独修复）
