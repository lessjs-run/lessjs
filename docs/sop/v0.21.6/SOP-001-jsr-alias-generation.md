...
ADR 0015 introduced auto-generated Vite resolve aliases for `@lessjs/core/*`
...
const selfUrl = import.meta.url; // https://jsr.io/@lessjs/adapter-vite/0.21.5/src/index.ts
// path resolution fails because it's not file://
```

When `@lessjs/adapter-vite` is consumed via JSR (the normal user path), the
...
2. Generate a minimal set of aliases for `@lessjs/core` subpath exports
...
function getPackageRoot(): string {
  ...
  // e.g., https://jsr.io/@lessjs/adapter-vite/0.21.5/src/index.ts
  // → https://jsr.io/@lessjs
...
deno run -A jsr:@lessjs/create test && cd test && deno task build
