...
```
你是 LessJS（https://github.com/open-element/open-element）的资深架构审计专家。请对当前仓库进行一次全面深度审计。

## 项目背景

LessJS 是一个 DSD-first Web Components 框架，Deno monorepo，通过 JSR 发布 @openelement/* 系列包。
...
- 新增 @openelement/signals 作为 @openelement/core 的显式依赖
...
- 包依赖图是否有循环引用（18 packages, @openelement/signals → @openelement/core → ...）
