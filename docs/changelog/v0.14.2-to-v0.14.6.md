...
| --- | ------------------------------------------------------------------------------------ | --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| B-1 | `bindEvents()` 原型污染 / Prototype pollution in `bindEvents()`                      | `packages/core/src/context.ts`    | 过滤 `__proto__`/`constructor`/`prototype` 键，阻止通过属性绑定注入原型链 / Filter `__proto__`/`constructor`/`prototype` keys to prevent prototype chain injection via property binding                                        |
| B-2 | `connectedCallback` 递归 / `connectedCallback` recursion                             | `packages/core/src/context.ts`    | 添加 `__bindEventsDone` 幂等守卫，防止 `bindEvents()` 在同一元素上重复触发 `connectedCallback` / Add `__bindEventsDone` idempotency guard to prevent `bindEvents()` from re-triggering `connectedCallback` on the same element |
...
| S-4  | `adapter-registry` 单例警告 / Singleton warning                                | 添加 `@warning` JSDoc：模块级 `_adapter` 单例在多 Bundle 场景下可能失败，推荐使用 `@openelement/app` 统一入口 / Warn that module-level singleton can fail in multi-bundle scenarios; recommend `@openelement/app` as unified entry |
...
| S-10 | `c.req.param()` 防御性回退 / Defensive fallback                                | 生成代码中 `c.req.param()` 添加 `                                                                                                                                                                                                  |
...
| S-13 | `readWorkspacePackageVersion` 单仓路径假设 / Monorepo path assumption          | 添加 `import.meta.url` 路径推断作为回退，支持 `@openelement/adapter-vite` 在单仓外使用 / Add `import.meta.url`-based path inference fallback for out-of-monorepo usage                                                             |
...
**EN:** The most critical starting point was v0.14.3's prototype pollution fix in `bindEvents()`, which could have allowed arbitrary property injection via `__proto__`/`constructor`/`prototype`. The `connectedCallback` recursion guard and CSP nonce validation (v0.14.5) complete the security surface hardening.

**ZH:** 最关键的起点是 v0.14.3 中 `bindEvents()` 的原型污染修复——原本可能通过 `__proto__`/`constructor`/`prototype` 注入任意属性。`connectedCallback` 递归守卫和 CSP nonce 校验（v0.14.5）完成了安全面加固。
...
| P0                | `@openelement/create` 远程版本解析不可靠 / Remote version resolution unreliable                  | ❌ 未修复 / Open |
