...
- **Core**: `DsdElement` now exposes `update()` for native string-rendered
  ...
  alias for packages such as `@lessjs/rpc`.
...
- **Status**: ACCEPTED / IMPLEMENTING → **IMPLEMENTED**.
...
- **Status**: 执行中 → **已完成**. Verified zero `from 'lit'` imports in
  `@lessjs/ui/src/`, zero `DsdLitElement` references outside migration
...
### Residual — Deferred to v0.21+

- Split `less-layout.ts` (964 lines) into layout/navigation modules.
- `@lessjs/adapter-lit` DSD-path deprecation timeline.
