...
将 `@lessjs/core` 拆分为构建时工具 + 零运行时。Core 在 SSG 构建后不进入客户端 bundle。页面只携带：
...
`@lessjs/rpc` 基于 Hono RPC 的类型安全客户端。不再使用代码生成，直接从 Hono 路由定义推断类型：
...
| 包                  | 版本  | 说明                       |
| ------------------- | ----- | -------------------------- |
| @lessjs/core        | 0.5.0 | Vite 插件 + SSG + DSD 渲染 |
| @lessjs/rpc         | 0.5.0 | 类型安全 RPC 客户端        |
| @lessjs/ui          | 0.5.0 | Lit Web Component 库       |
| @lessjs/adapter-lit | 0.5.0 | Lit SSR 适配器             |
| @lessjs/create      | 0.5.0 | 项目脚手架 CLI             |
