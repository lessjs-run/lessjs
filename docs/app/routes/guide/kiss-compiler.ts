    ...
    return html`
      <kiss-layout currentPath="/guide/kiss-compiler">
        <div class="container">
          ...
          <h2>Context</h2>
          <p>
            KISS currently depends on <code>lit</code> (npm:lit) for component authoring. This brings a
            58kb gzip runtime, @lit-labs/ssr for server rendering (with CJS polyfill), hydration ceremony,
            and dprint fmt panics on tagged template literals.
          </p>

          <h2>Proposal</h2>
          <p>
            Introduce <code>.kiss</code> files — a component format purpose-built for KISS. A compiler
            transforms <code>.kiss</code>
          ...
          <table>
            ...
            <tr>
              <td>SSR</td>
              <td>@lit-labs/ssr + DOM shim</td>
              <td>template.innerHTML (sync)</td>
            </tr>
            <tr>
              <td>Hydration</td>
              <td>DSD + hydrate() + order bug</td>
              <td>template.cloneNode (no hydration)</td>
            </tr>
            <tr>
              <td>Polyfills</td>
              <td>node-domexception CJS shim</td>
              <td>none needed</td>
            </tr>
            <tr>
              <td>Build</td>
              <td>esbuild decorator transform</td>
              <td>standard TS/JS only</td>
            </tr>
          </table>
          ...
          <p>
            <code>vite.config.ts</code> option: <code>compiler: 'lit' | 'kiss' | 'auto'</code>. Lit
            support retained throughout v0.x. v1.0 defaults to .kiss compiler.
          </p>
