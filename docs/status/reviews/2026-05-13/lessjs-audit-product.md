...
| 包                        | 评价 | 备注                                                   |
| ------------------------- | ---- | ------------------------------------------------------ |
| @openelement/core         | 优秀 | 导出路径、API 示例、渲染模型完整                       |
| @openelement/adapter-vite | 优秀 | 插件表、配置选项表、SSG 三阶段说明清晰                 |
| @openelement/adapter-lit  | 良好 | DSD Hydration Mixin 用法完整                           |
| @openelement/content      | 良好 | 推荐用法 + 独立用法双路径                              |
| @openelement/i18n         | 良好 | 与 app 集成和独立使用都有说明                          |
| @openelement/app          | 良好 | 伞包用法、选项表完整                                   |
| @openelement/ui           | 良好 | 8 个组件清单 + 设计令牌说明                            |
| @openelement/signals      | 一般 | API 示例完整但缺少高级用法（如 islandEffect 生命周期） |
| @openelement/rpc          | 良好 | 用法 + 重试配置 + 失败数据流完整                       |
| @openelement/create       | 良好 | 项目结构 + 添加 Islands/API 路由说明                   |
...
| @openelement/core LICENSE         | ✅ MIT  | 与根目录一致                          |
| @openelement/adapter-vite LICENSE | ✅ MIT  | 与根目录一致                          |
| @openelement/adapter-lit LICENSE  | ✅ MIT  | 与根目录一致                          |
| @openelement/content LICENSE      | ✅ MIT  | 与根目录一致                          |
| @openelement/i18n LICENSE         | ✅ MIT  | 与根目录一致                          |
| @openelement/app LICENSE          | ❌ 缺失 | `packages/app/` 目录下无 LICENSE 文件 |
| @openelement/ui LICENSE           | ✅ MIT  | 与根目录一致                          |
| @openelement/signals LICENSE      | ✅ MIT  | 与根目录一致                          |
| @openelement/rpc LICENSE          | ✅ MIT  | 与根目录一致                          |
| @openelement/create LICENSE       | ✅ MIT  | 与根目录一致                          |

**关键问题**：`@openelement/app` 缺少 LICENSE 文件。该包的 `deno.json` 中 `publish.include` 声明了 `["src/**", "deno.json", "LICENSE"]`，但实际文件不存在，可能导致 JSR 发布包含空 LICENSE 或发布失败。
...
### 4.1 脚手架体验 (`@openelement/create`)

**优点**：

- 一行命令 `deno run -A jsr:@openelement/create my-app` 极简
...
| 包                        | README 版本 | 实际版本 | 偏差     |
| ------------------------- | ----------- | -------- | -------- |
| @openelement/core         | 0.13.0      | 0.14.0   | 1 minor  |
| @openelement/adapter-vite | 0.3.0       | 0.14.0   | 11 minor |
| @openelement/adapter-lit  | 0.8.0       | 0.14.0   | 6 minor  |
| @openelement/content      | 0.3.3       | 0.14.0   | 11 minor |
| @openelement/i18n         | 0.1.1       | 0.14.0   | 13 minor |
| @openelement/app          | 0.3.1       | 0.14.0   | 11 minor |
| @openelement/ui           | 0.7.1       | 0.14.0   | 7 minor  |
| @openelement/signals      | 0.6.2       | 0.14.0   | 8 minor  |
| @openelement/rpc          | 0.6.1       | 0.14.0   | 8 minor  |
| @openelement/create       | 0.7.0       | 0.14.0   | 7 minor  |
...
| #  | 问题                                                                                                                          | 文件路径                              | 严重度 |
| -- | ----------------------------------------------------------------------------------------------------------------------------- | ------------------------------------- | ------ |
| V1 | publish.yml 中 @openelement/adapter-vite 出现两次（第 63 和 66 行），重复发布                                                 | `.github/workflows/publish.yml:63,66` | 高     |
...
| V5 | `publish:signal` task 名与包名 `@openelement/signals` 一致，但 README 和架构图使用 `@openelement/signals`（复数），命名不一致 | `deno.json:45` vs `README.md:47`      | 高     |

### 5.4 包命名不一致

| 位置                                   | 名称                           |
| -------------------------------------- | ------------------------------ |
| `packages/signals/deno.json`           | `@openelement/signals`（单数） |
| README.md 架构图和版本表               | `@openelement/signals`（复数） |
| `deno.json` publish task               | `publish:signal`（单数）       |
| `@openelement/create` 生成的 deno.json | `@openelement/signals`（单数） |
| README.en.md                           | `@openelement/signals`（复数） |

JSR 发布名为 `@openelement/signals`，但文档中统称 `@openelement/signals`。这会导致用户搜索不到包或导入了错误的包名。
...
2. **修复 publish.yml 重复发布**：删除第 66 行重复的 `@openelement/adapter-vite` 发布
   - 文件：`.github/workflows/publish.yml:66`

3. **补充 @openelement/app LICENSE 文件**：创建与根目录一致的 MIT LICENSE
...
主要短板集中在**工程治理**层面：README 版本信息严重过时、@openelement/app 缺失 LICENSE、signal/signals 命名不一致、publish.yml 重复发布等问题直接影响用户信任。开源社区治理文件（CODE_OF_CONDUCT、CONTRIBUTING、SECURITY）全部缺失，对于一个已发布到 0.14.0 的框架来说不够成熟。
