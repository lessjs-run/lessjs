...
- **问题**: `@openelement/core` 依赖 `parse5` (npm: specifier)，违反了 JSDoc 中的"零 npm: specifiers"声明（core/src/index.ts 第7行）
...
- M1: `parse5` 在 `@openelement/core` 中违反"零 npm: specifiers"声明
- M2: 无第三方扩展的正式插件 API
- M3: `app/components/` vs 内联路由组件的指导不清晰

**改进建议**:

1. 将 `parse5` 从 `@openelement/core` 移除
